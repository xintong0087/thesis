...
From another point of view, MLR and QPR are two neural networks with no hidden layers. 
For a traditional regression model, the numbers of neurons in its input layer equals to the cardinality of its regression bases.
Treating regression metamodels as shallow neural networks allows for direct comparisons with DNNs.
...
\footnotetext{This row summarizes the results of the well-trained RNN, where the ones suffers from the vanishing and exploding gradient issue are removed from the averages.}
...
\begin{enumerate}
    ...
    This makes a LSTM network more capcable of learning long-term dependencies. 
...
\end{enumerate}
In summary, the network depth ensures successful learning of the training dataset, and the network architecture serves as a prior knowledge that guides the optimization algorithm to find a solution that generalizes well to unseen data.
