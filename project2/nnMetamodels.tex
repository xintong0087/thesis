...
\begin{algorithmic}[1] \label{alg2:twoStageProcedure}
    ...
    \begin{enumerate} [label=\alph*., itemsep=0pt, parsep=0pt, topsep=0pt]
        ...
        It is standard practice in supervised learning research to transform the stock prices $\bS^{(i)}$ to log returns $\mathbf{X}^{(i)}$,
        $$
        \mathbf{X}^{(i)} = \left(\ln\left(\frac{S_{1}^{(i)}}{S_{0}^{(i)}}\right), \ln\left(\frac{S_{2}^{(i)}}{S_{1}^{(i)}}\right), \ldots, \ln\left(\frac{S_{T}^{(i)}}{S_{T-1}^{(i)}}\right)\right),
        $$
