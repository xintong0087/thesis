...
From a neural network of view, MLR and QPR are two neural networks with no hidden layer. 
...
Treating a regression as a shallow neural network, i.e., a neural network with only an input layer and an output layer, allows for direct comparisons with DNNs.
The capacity of a regression metamodel is the product of the number of neurons in its input layer and its output layer.
Since our target variable is a scalar, the output layer has only one neuron.
The capacity of a regression metamodel therefore equals to the size of its regression basis.
On the other hand, the three DNN metamodels have several hidden layers and are much more complex than the two regression metamodels.
Our FNN has two hidden layers before the output layer.
Its last hidden layer serves as an automatic feature extraction layer that transforms the 240-dimension feature vector into a 32-dimension vector that can directly relate to the target variable.
...
For the RNN, two of its hidden layers are recurrent layers that capture the temporal dependency in the feature, and the last hidden layer is a standard feedforward layer that transforms the previous hidden states into a 32-dimension vector for target prediction.
Our LSTM has the same architecture as the RNN but with LSTM cells in place of the RNN cells.
...
\begin{enumerate}
    ...
    The last row of Table~\ref{tab:gmwb_arch} shows the average squared errors of the good RNN metamodels that are successfully trained. 
