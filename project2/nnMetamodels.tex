...
In a nested simulation, a simulation model is used to generate a large number of outer scenarios, and each scenario is then used as an input to another simulation model, referred to as a inner simulation model.
...
We see from the above modeling steps that the status of a generic VA contract is summarized by a triplet $(S_t,F_t,G_t)$ which evolution is driven by the stochasticity of $S_t$.
...
Consider a VA contract which delta hedge portfolio at any time~$t$, $t=0,1,\ldots,T-1$, consists of $\Delta_t$ units in the underlying stock and $B_t$ amount of a risk-free zero-coupon bond maturing at time $T$.
...
\footnotetext{This row summarizes the error metrics of the well-trained RNN, where the ones suffers from a vanishing gradient issue are removed from the averages.}
...
In our experiments, there are important differences among the three DNN models due to having different neural network architectures.
...
The intuition behind the RNN and LSTM metamodels is briefly mentioned in~\ref{subsec:LSTM}.
...
\begin{enumerate}
    ...
    As a result, the half length of the $95\%$ confidence band of the RNN's training error is much larger than all other metamodels. 
...
Instead, we recommend to simulate the true contract losses on a subset of training scenarios to evaluate the metamodels' generalization ability.
...
A single-stage procedure with a regular LSTM metamodel is particularly superior to the two-stage procedure, as it is able to achieve a higher accuracy as the standard procedure with a much smaller amount of simulation budget.
...
This is a an encouraging evidence that DNN metamodels can be used to improve the efficiency of Monte Carlo simulation for quantitative risk management tasks that require a computational-expensive simulation procedure.
