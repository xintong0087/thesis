...
\gls{dnn} have attracted attentions of researchers and practitioners due to their success~\citep{hastie2009elements,lecun2015deep} in solving real-world \gls{ml} tasks such as AlphaGo~\citep{silver2016mastering} and ChatGPT~\citep{chatgpt}.
...
For example,~\cite{luo2016understanding} show that adding synthetic label noise to the \gls{cnn} can improve its ability to capture global features. 
...
Estimating tail risk measures of portfolios of complex \gls{va} contracts usually requires nested simulation.
...
A prominent expirical proxy method discussed in the literature is the least squares \gls{mc} method~\citep{longstaff2001valuing}.
...
This chapter introduces a proxy model to reduce the computational requirements of inner simulations of \gls{va} contracts by replacing the inner simulation step of a nested simulation procedure. 
...
Borrowing terminologies from the \gls{ml} community, we can view a set of simulated outer scenarios and estimated hedging errors for those scenarios as \textit{features} and noisy \textit{labels}.
...
~\cite{liu2010stochastic} use stochastic kriging as a metamodel of \gls{mc} simulations to estimate the \gls{cvar} of a portfolio of derivative securities, and~\cite{gan2015valuation} use a stochastic kriging for an efficient valuation of large portfolios of \gls{va} contracts.
...
\begin{enumerate}
    \item  our metamodel has high-dimensional inputs. In \gls{ml} terminology, the features are high-dimensional vectors.
...
Section~\ref{sec2:problem-formulation} presents the problem settings for tail risk measures and dynamic hedging of \gls{va} contracts. 
...
In this section we present notations, problem settings, and a simulation model for risk estimation for hedging errors of \gls{va} contracts.
...
In Canada, the regulatory capital requirement for \gls{va} contracts is set based on \gls{cvar}s as prescribed in~\cite{osfi2017life}.
...
\begin{itemize}
    ...
    \item   $\Lhat_i^{MC}$ is a standard \gls{mc} estimator of the true loss for scenario $\bS^{(i)}$. 
...
To make our examples more realistic, the \gls{va} contracts are also subject to a dynamic lapse model with the following key features~\citep{naic2021}:
\begin{enumerate}
...
The risk-free rate is $0.2\%$ per month and the underlying asset $S_t$ is modeled by a \gls{rsgbm} with parameters specified in Table~\ref{tab:regime_params}.
...
\begin{table}[ht]
    ...
    \begin{tabular}{lc}
        ...
        \bottomrule
    \end{tabular}
    \caption{Real-world parameters for the RS-GBM model (monthly rates)}
...
\gls{fnn} (Equation~\refeq{eq:fnn}) is a generic neural network while \gls{rnn} (Equation~\refeq{eq:rnn}) and \gls{lstm} (Equation~\refeq{eq:lstm}) are specialized models to accommodate the sequential structure of our time-series features.
A $\tanh$ activation function is used for \gls{rnn} and \gls{lstm} layers, and a \gls{relu} activation function is used for the fully-connected layers.
...
Following the convention in \gls{ml} literature, the three \gls{dnn} metamodels are said to have much higher \textit{model capacities}.
...
Specifically, following the convention in \gls{ml} literature, we split this dataset into three parts: The training, validation, and test sets have $\num{90000}$, $\num{5000}$, and $\num{5000}$ samples (representing $90\%$, $5\%$, and $5\%$ of the dataset), respectively.
...
In our numerical experiments, we are able to use \gls{mc} simulation to generate a true dataset that approximates the true hedging losses with a high degree of precision, and, as a result, we are able to explore the important question of whether deep learning models are indeed capable of learning the true feature-label relationship from noisy training labels.
...
By showcasing the resilience of these models, our study aims at encouraging regulatory bodies to recognize the value and applicability of deep learning metamodels in financial risk management, and it provides informed suggestions and guidance for the incorporation and oversight of advanced deep learning metamodel in \gls{mc} Simulation in financial applications.
Our findings are particularly insightful in this context.

In our numerical experiments, a \gls{lstm} metamodel is resilient to a moderate level of noise in training labels and is able to make accurate predictions. 
This is an encouraging evidence that \gls{dnn} metamodels can be used to improve the efficiency of \gls{mc} simulation for quantitative risk management tasks that require a computational-expensive simulation procedure.
...
Furthermore, our numerical results provide evidences for adopting \gls{dnn} metamodels in \gls{mc} simulation for risk management tasks.
