...
\section{Problem Formulation} \label{p2:problem-formulation}
...
Figure~\ref{fig:QQ_REG} and Figure~\ref{fig:QQ_NN} shows the QQ plots between the \textbf{true} labels and the predicted losses for the regression metamodels and the neural network metamodels, respectively.
...
For some macro replications (as shown in Figure~\ref{subfig:badRNN}), the RNN metamodels' predictions are far from the true losses, especially on the tails.
...
Figure~\ref{fig:CVaR95} shows the 95\%-CVaR estimates for all five metamodels with different safety margins, averaged over 50 macro replications.
...
\begin{table}[ht!]
    ...
    \begin{tabular}{lccccc}
        \toprule
        \textbf{Model}      & \textbf{Noise level of training labels}   & \textbf{Training error}   & \textbf{Test error}   & \textbf{True error}\\
        \midrule
...
The 4th row of Table~\ref{tab:lstm_sens} shows the performance of the regular LSTM metamodel trained with $M=100\!,000$ samples with different noise levels.
...
\begin{figure}[ht!]
    ...
    \begin{subfigure}{0.48\textwidth}
        ...
        \caption{With high noise training labels ($N=1$)}
    \end{subfigure}
...
The standard procedure shown in Figure~\ref{fig:CVaRsingleStage} uses the noisy loss labels in the training dataset to estimate the CVaR directly, which uses the same simulation budget as the single-stage procedure.
...
However, results in Figure~\ref{fig:CVaRsingleStage} suggest that the FNN metamodel should not be used in the single-stage procedure.
...
Figure~\ref{fig:gammaConvergence} shows the log-log plots of the relative root mean square errors (RRMSE) between the LSTM metamodel CVaR predictions and the true CVaR against the total simulation budget $\Gamma$.
...
Figure~\ref{fig:mnConvergence} shows the log-log plots of the RRMSE between the LSTM metamodel CVaR predictions and the true CVaR against the simulation budget.
...
For an LSTM metamodel, increasing the data quality of the training labels has diminishing returns, which is consistent with the results in Figure~\ref{fig:mnConvergence}.
