...
Many procedures are proposed in the literature, but they are not directly comparable due to having different error metrics, different assumptions, and different numerical examples used in their studies.
...
In terms of finite-sample performance, different studies propose different examples in their numerical experiments, which introduces unfair advantages for certain simulation procedures over the others. 
...
The simulation budget $\Gamma$ is a sum of the computational budget at each level, that is, $\Gamma = \sum_{\upsilon=0}^{\Upsilon} M_{\upsilon} \cdot N_{\upsilon}$.
...
After a supervised learning metamodel is trained, it can be used to form predictions for all $X \in \mathcal{X}$.
...
\begin{align}
    ...
    & \text{subject to} ~~~ M \cdot N = \Gamma, 
\end{align}
where $\Gamma$ is the a simulation budget for a nested simulation procedure.
...
In the following, we analyze the convergence of the standard nested simulation procedure for a smooth and a Lipschitz continuous $h$ by using both decompositions.
...
Instead, we use the a Taylor expansion of the smooth function $h$ to analyze the convergence of the standard nested simulation procedure.
...
The theoretical framework has lend itself for a comparison of the asymptotic convergence behavior for different nested simulation procedures.
...
To examine the sensitivity of the empirical convergence behavior to the asset model, we consider a stochastic volatility model of a Heston type~\citep{heston1993stochastic}.
...
The cost associated with pooling makes a major impact on the overall computaional complexity of a metamodel-based nested simulation procedure.
...
\begin{itemize}
    ...
    In our implementaion of kNN, the cross-validation is conducted by using a grid search with a search space which cardinality does not grow with the simulation budget $\Gamma$.
