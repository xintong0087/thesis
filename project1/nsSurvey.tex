...
Given a fixed computational budget, the nestzed simulation procedure should balance the number of outer scenarios and the number of inner replications to maximize computational efficiency.
...
Other risk measures of interest not in the nested expectation form include Value at Risk (VaR) and Conditional Value at Risk (CVaR)~\citep{hardy2022quantitative}. 
...
\textcolor{red}{For the nested expectation form $\rho = \mathbb{E}[h(L)]$, the MLMC method selects an approximation level $L$ with sufficient accuracy, and it uses the difference between the approximations to reduce the variance of the estimator.}
\textcolor{red}{\sout{The MLMC method estimates $\rho$ with an MLMC estimator:}}


\begin{equation*}
    \hat{\rho}^{\text{MLMC}}_\Gamma = \sum_{\upsilon=0}^{\Upsilon} \left( \frac{1}{M_{\upsilon}} \sum_{i=1}^{M_{\upsilon}} h(\hat{L}_{N_{\upsilon}}(X_{i, \upsilon})) - \frac{1}{M_{\upsilon-1}} \sum_{i=1}^{M_{\upsilon-1}} h(\hat{L}_{N_{\upsilon-1}}(X_{i, \upsilon-1})) \right), \quad X_{i, \upsilon} \sim F_X,
\end{equation*}
...
\begin{itemize}
    ...
    \item   kNN~\citep{mack1981local}:
            ...
            The kNN method is implemented as a metamodel for numerical experiments in~\cite{hong2017kernel}, but the convergence analysis is not provided.
    ...
    \item   KRR:
            ...
            ~\cite{wang2022smooth} provides a convergence analysis for the AE (Equation~\eqref{eq1:ae}), and the convergence analysis is provided for a smooth, a Lipschitz continuous, and an indicator $h$, VaR, and CVaR.
\end{itemize}
...
Among the supervised learning models we consider, multiple linear regression is parametric, while kernel smoothing and kernel ridge regression are non-parametric.

We aim to minimize the MSE of the supervised learning-based nested simulation estimators $\hat{\rho}^{\text{SL}, \text{Train}}_{M, N}$ and $\hat{\rho}^{\text{SL}, \text{Test}}_{M, N, M'}$ subject to the total simulation budget $\Gamma$.
...
For the more complex metamodels, the convergence analysis of their MSEs is more challenging.
...
While most of the literature focuses on the MSE of the estimator of $\rho$,~\cite{wang2022smooth} analyze the asymptotic convergence of the estimator of $\rho$ in terms of AE.
...
Equation~\eqref{eq1:var-sns-smooth} directly follows from Assumption~\ref{as1:sns} and the fact that $\hat{L}_N$ is a standard MC estimator of $L$.
...
A kNN regression can be used as a non-parametric metamodel that estimates the conditional expectation of $Y$ given $X$ by averaging the $Y$ values of the $k$ nearest neighbors of $X$.
...
The following lemma from~\cite{mack1981local} characterizes pointwise convergence of the kNN estimator in terms of its bias and variance at point $x$.
...
A total of 5 nested simulation procedures are considered, namely a standard nested simulation procedure, a  regression-based, a kernel smoothing-based, a likelihood ratio-based, and a kernel ridge regression-based nested simulation procedure.
...
\textcolor{red}{We note that the MLMC procedure focuses on adaptive allocation of the simulation budget and is fundamentally different from supervised learning-based nested simulation procedures. And their results are not directly comparable.}
