...
This bound can be strengthened by using the bias-variance decomposition in Equation~\ref{eq1:mse-sns2}.
...
\begin{align} \label{eq1:bias-sns-lip}
    ...
    & \leq K \sqrt{\mathbb{E} \left[ \left( \bar{Z}_N \right)^2 \right]} = \mathcal{O}(N^{-1})
\end{align}
...
Hence, the convergence order of the standard nested simulation procedure for Lipschitz continuous $h$ is the same as the one for smooth $h$.
