...
Many procedures have been proposed in the literature, but they are not directly comparable because they use \textcolor{red}{\sout{different error metrics,}} different assumptions and different numerical examples in their studies.
...
\textcolor{red}{\sout{Furthermore, the existing literature has chosen different performance metrics to evaluate the asymptotic convergence of nested simulation procedures.
...
This phenomenon is due to the fact that the convergence analysis for the \gls{krr}-based nested simulation procedure is shown in terms of \textcolor{red}{\gls{ae}, a different error metric than \gls{mse}.}
\textcolor{red}{In the appendix,} Section~\ref{appendix:connection-mse-absolute-error} includes a discussion of the connection between the \gls{mse} and the \gls{ae} of the estimator in the context of nested simulation procedures.
...
\textcolor{red}{Theoretical properties of different metamodels lead to different convergence results.}
...
\begin{itemize}
    \item provide the asymptotic convergence results for the standard nested simulation procedure, \textcolor{red}{and}
    \item \textcolor{red}{\sout{show the connections between the convergence in \gls{mse} and the convergence in probabilistic order for \gls{ae} in the context of nested simulation procedures, and}}
...
\textcolor{red}{In Section~\ref{appendix:connection-mse-absolute-error}, we illustrate the connection between the convergence in \gls{mse} and the convergence in \gls{ae}.}
...
\textcolor{red}{In Equation~\eqref{eq1:mse-sns}, the first inequality follows from Cauchy-Schwarz inequality.
...
\end{equation*}
\textcolor{red}{By setting $x_1 = \rho_{M,N}- \rho_M$, $x_2 = \rho_M - \rho$, $y_1 = y_2 = 1$, we have }
\begin{align*}
...
\textcolor{red}{\sout{As stated in Theorem, the convergence in \gls{mse} automatically implies the convergence in probabilistic order.}}
...
\textcolor{red}{\sout{Immediately from Theorem, the convergence in \gls{mse} automatically implies the convergence in probabilistic order.}}
...
Furthermore, the convergence analysis of the \gls{krr}-based nested simulation procedure is shown in terms of \gls{ae} rather than in \gls{mse}, which is due to the \textcolor{red}{analytical properties} of the KRR metatmodel.
...
\textcolor{red}{The empirical convergence rates reported in the graph legends are higher than the actual rates after regression-based nested simulation procedure reaches the asymptotic convergence regime.}
...
\textcolor{red}{In terms of finite-sample behavior, this study provides a comprehensive comparison of different nested simulation procedures, and the findings can be used to guide the choice of a proper nested simulation procedure for a given nested estimation problem.}
