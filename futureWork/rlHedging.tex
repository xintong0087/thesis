...
Instead of retraining a \gls{drl} model from the scratch when market conditions shift, \gls{tl} enables the reuse of pre-trained models, facilitating a faster adaptation to new environments while avoiding overfitting to past conditions.
...
Section~\ref{sec:PPO} introduces the \gls{ppo} algorithm and its application to the hedging of \gls{va} contracts.
...
By formulating the \gls{va} hedging problem as an \gls{mdp}, we can utilize \gls{rl} algorithms to learn optimal hedging policies directly from data.
...
A \gls{mdp} provides a mathematical framework for solving sequential decision-making tasks in an environment where outcomes are partly random and partly under the control of a hedging agent. 
...
In this section, we reformulate the hedging environment of \gls{va} contracts from Section~\ref{subsec:VApayout} as an \gls{mdp} and define the key components of the \gls{mdp}.
...
\begin{enumerate}
    ...
    \end{cases}
    $$
    and $M_q$, $D_q$, $L_q$, and $U_q$ are the model parameters taken from National Association of Insurance Commissioners' Valuation Manual 21~\citep{naic2021}.
...
\end{align}
where the equality holds by a rearrangement of terms and a telescopic sum simplification of $e^{-rt}B_t$, $t=0,\ldots,T-1$, and $C_t := C \cdot S_{t-1} \cdot |\Delta_t - \Delta_{t-1}|$ is the transaction cost at time $t$~\citep{garleanu2013dynamic}.
...
\section{Existing RL Algorithms and Their Limitations} \label{sec:RLAlgorithms}

\subsection{Model-Based RL: From AlphaZero to Deep Hedging}
\gls{drl} first attracted attention when AlphaGo~\citep{silver2016mastering} surpasses human in playing Go. 
...
In essence, the AlphaZero agent use a neural network to parameterize the policy and the value function.
...
They are induced by a planning algorithm, i.e., \gls{mcts}.
...
Its first two terms measure the similarity between $f_{\theta}(s)$, the learned policy and value function, with the action and game outcome planned by the \gls{mcts}.
...
In contrast to AlphaZero, deep hedging~\citep{buehler2019deep} uses \gls{mc} simulations to generate sample paths of asset dynamics.
...
A common choice of $\rho$ is a \gls{cvar}.
In practice, the neural network $f_{\theta}$ is trained to minimize the empirical \gls{cvar} over a batch of trajectories.
...
Broadly speaking, model-free \gls{rl} can be categorized into two types: value-based \gls{rl} and policy-based \gls{rl}.
...
\section{Hedging VA contracts with a PPO Agent} \label{sec:PPO}
...
In our implementation, the advantage function is estimated following the Generalized Advantage Estimation method proposed by~\cite{schulman2015high}.
...
\begin{table}[ht!]
    ...
    \begin{tabular}{ll} 
        ...
        \bottomrule
    \end{tabular}
    \caption{Model hyperparameters and GMMB contract specifications} 
...
\begin{figure}
    ...
    \caption{Hedging performance of deep hedging with TL}
...
The figure plot the 95\%-\gls{cvar} of hedging errors against training epochs for both agents.
...
Figure~\ref{fig3:ppo_GMWB} illustrates the hedging performance of the recurrent \gls{ppo} agent for hedging \gls{va} contracts with a \gls{gmwb} rider.
...
By addressing these objectives, the research aims at advancing the field of financial risk management through the development of adaptive and efficient hedging strategies for \gls{va} contracts. 
The integration of \gls{tl} into \gls{ppo} agents has the potential to substantially reduce computational costs and improve the robustness of hedging policies in dynamic market environments.