\chapter{Model-Free Deep Hedging of Variable Annuities} \label{chap:project3}

\section{Introduction}

In the evolving landscape of financial markets, insurance products such as variable annuities (VAs) have gained significant interest due to their ability to provide both investment growth and guaranteed benefits. 
...
Reinforcement Learning (RL) has emerged as a powerful tool for addressing such complexities. With the recent advancement in deep learning, deep neural network (DNN) architectures have been successfully applied to RL problems, leading to the development of deep reinforcement learning (DRL) algorithms. 
...
By intergrating deep neural network, DRL extends this framework by enabling the agent to handle high-dimensional and complex environments. 
...
Training a DRL agent from scratch requires a large dataset along with considerable computational time for exploration and learning.
This is especially problematic for variable annuities, which are long-term insurance products that are sensitive to market fluctuations and underlying actuarial assumptions.
Given that financial markets are highly dynamic, frequent retraining to account for new market regimes becomes necessary, which can lead to prohibitively high costs and computational inefficiency.
...
Transfer learning (TL) offers a compelling solution to the challenges posed by existing DRL approaches. 
The core idea of TL is to leverage knowledge from previously solved tasks and apply that knowledge to new but related tasks.
In the context of financial modeling, this means that instead of training an RL agent from scratch every time underlying assumptions change, the model can utilize the knowledge gained from past market environments to accelerate the learning process under the new environment.
...
Instead of retraining a deep RL model from scratch when market conditions shift, TL enables the reuse of pre-trained models, facilitating faster adaptation to new environments while avoiding overfitting to past conditions.
In the risk management of variable annuities, transfer learning enables quicker adaptation to changing financial and actuarial assumptions by reusing information obtained from another set of related assumptions, such as the management of similar financial instruments or different market regimes. 
...
More specifically, a DRL agent trained to manage variable annuities for a GMMB contract can apply its learned strategies to a GMWB contract with the same financial assumptions. 
...
This approach significantly reduces the computational costs associated with the training of new VA products with different assumptions and improves the generalization of the model to unseen market conditions.
By integrating transfer learning with existing DRL frameworks, we can eliminate the inefficiencies of training models from scratch, provide faster adaptability to new market conditions, and enhance the scalability and computational efficiency of financial models. 
This approach offers a more robust and practical solution for managing variable annuities in dynamic and volatile financial markets.

The rest of the paper is organized as follows: 

\section{Markov Decision Process for Hedging VAs}
...
\section{Exising RL Algorithms and Their Limitations}
...
For example, deep hedging builds its own model of the hedging environment to plan the hedging strategy ahead of time using a neural network and a risk measure.
Designing a suitable risk measure for such a network is necessary but challenging.
...
A model-based RL uses a model of the environment that enables decision-making with future situations that have not been experienced by the agent.
However, the model bias can lead to suboptimal performance in the real environment.
In contrast, a model-free RL learns directly from the environment without relying on a model, which makes it more flexible and robust to model misspecification.
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=\textwidth]{./project3/figures/PPO_DH.png}
...
The experiment is conducted with the same simulation budget.

\begin{figure}[ht!]
    \centering
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\textwidth]{./project3/figures/PPOs_GBM.png}
        \caption{GBM}
    \end{subfigure}
    \hspace{1cm}
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\textwidth]{./project3/figures/PPOs_GBMRS.png}
...
\begin{figure}[ht!]
    \centering
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\textwidth]{./project3/figures/PPO_cost_001.png}
        \caption{$C = 0.001$}
    \end{subfigure}
    \hspace{1cm}
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\textwidth]{./project3/figures/PPO_cost_02.png}
        \caption{$C = 0.02$}
    \end{subfigure}
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=\textwidth]{./project3/figures/PPO_anchor.png}
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=\textwidth]{./project3/figures/PPO_GMWB.png}
