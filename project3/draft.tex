...
L   & = H_0 - e^{-rT} H^{bf}_T + \sum_{t=1}^{T-1} e^{-rt} HE_t + \sum_{t=1}^T e^{-rt} C_t - \sum_{t=1}^T e^{-rt} F_t\eta_n + e^{-rT} (G_t - F_t)^+ nonumber \\ 
    & = e^{-rT} (G_T - F_{T^+})^+ + \sum_{t=1}^T e^{-rt}  \left( \Delta_{t-1} (S_{t-1} - e^{-r} S_t) + C_t - F_t\eta_n \right) \label{eq3:lossrv}
\end{align}
where the equality holds by a rearrangement of terms and a telescopic sum simplification of $e^{-rt}B_t$, $t=0,\ldots,T-1$, and $C_t := C \cdot S_{t-1} \cdot (|\Delta_{t-1}| + 0.01 \Delta_{t-1}^2)$ is the tranaction cost at time $t$~\cite{garleanu2013dynamic}.
...
In a VA hedging problem the policy outputs the hedging weight $\Delta_t$ given the observed state $\mathbf{s}_t$.
...
Aside from the previous hedging weight $\Delta_{t-1}$, the transition probabilities are independent of the actions.
...
\begin{align}
    \mathcal{R} & = \sum_{t=0}^{T-1} \gamma^t R_t \\
    \gamma      & = e^{-r}, \nonumber
\end{align}
...
\begin{equation} \label{eq3:rewardGMWB}
    R_t = F_{t-1}\eta_n - \Delta_{t-1} |S_{t-1} - e^{-r}S_{t}| - C_{t-1}, \quad t \in \{1,\ldots,T \}
\end{equation}
...
\begin{equation} \label{eq3:rewardGMMB}
    R_t = 
    \begin{cases}
    F_{t-1}\eta_n - \Delta_{t-1} |S_{t-1} - e^{-r}S_{t}| - C_{t-1},                         & t\in \{1,\ldots,T-1 \} \\
    F_{t-1}\eta_n - \Delta_{t-1} |S_{t-1} - e^{-r}S_{t}| - C_{t-1} + (G_T - F_{T^+})^+      & t = T
    \end{cases}
\end{equation}
...
\subsection{From AlphaZero to Deep Hedging}
...
Soon after, AlphaZero~\citep{silver2016mastering} emerged as an RL algorithm that improves over self-plays.
...
\begin{equation}
    (\mathbf{\pi}, v) = f_{\theta}(s), \quad L(\theta) = (z - v)^2 - \mathbf{p}^T \log \mathbf{\pi} + c||\theta||^2,
\end{equation}
...
A planning algorithm like the MCTS has to be present due to the large search space in the game of Go.
