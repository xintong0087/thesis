...
Chapter~\ref{chap:project2} introduces a metamodeling-based nested simulation framework for dynamic hedging of VAs that approximates the inner simulation by an LSTM metamodel.
It shows that LSTMs, as a type of gated RNNs, are well-suited as metamodels for Monte Carlo simulation of financial time series and can capture the long-term dependencies in the simulation dataset.
...
This approach can significantly reduce the amount of training data needed, training time and computational resources while enhancing model generalization.
Instead of starting from scratch, a new DNN metamodel can be built on top of a pre-trained metamodel and fine-tuned on new data.
