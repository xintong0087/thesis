...
\begin{itemize}
    \item Outer: underlying risk factors (outer scenarios), $X_i \sim F_X$
    \item Inner: scenario-wise losses (inner replications), $Y_{ij} \sim F_{Y|X_i}$
    \item Total simulation budget: $\Gamma = M \cdot N$
\end{itemize}

\vspace{10pt}
\textbf{Computationally expensive}
...
\note{The nested structure is due to the fact that the loss random variable is in the form of a conditional expectation of the portfolio loss given the underlying risk factors. \\}
\note{In our problems, the loss random variable $L$ depends on the risk factors $X$ and is the conditional expectation of the portfolio loss given $X$. \\}
\note{More specifically, $X$ is $d$-dimensional and $L$ is 1-dimensional. \\}
\note{Estimating a risk measure, $\rho(L)$, involves two levels of Monte Carlo simulations: outer level generates underlying risk factors, and inner level generates scenario-wise samples of portfolio losses. \\}
\note{The nested structure makes the estimation computationally expensive. \\}
\note{In this thesis, we focus on examining and improving the efficiency of nested simulation procedures mostly using supervised learning metamodels.}
...
\note{A supervised learning based nested simulation procedure is implemented following these steps. \\}
...
\note{Then, the outer scenarios and inner sample means can be treated as features and labels. \\}
\note{Their relationship is then approximated by a supervised learning metamodel. \\}
\note{Finally, the trained metamodel is used to make predictions for all $X \in \mathcal{X}$. \\}
\note{In other words, instead of using the inner sample mean to estimate $L(X_i)$, we use the trained metamodel to make predictions of the portfolio loss. \\}
\note{These loss predictions are then used to calculate the risk measures of interest. \\}
\note{For most research in this area, the focus is on the convergence rate of the risk measure estimator in terms of the simulation budget. \\}
\note{However, in this thesis, we note that there are other important aspects to consider, such as the cost of pooling that may come from metamodel training and validation. }
...
\note{We start by comparing the asymptotic convergence rates of different nested simulation procedures. \\}
\note{These are the convergence rates of the MSE of the risk measure estimator when the total simulation budget $\Gamma$ goes to infinity, and when one follows the optimal growth order of $M$ and $N$. \\}
...
\note{It is achieved by only allowing $M$ to grow in the order of $\Gamma^{1}$. In their experiments, $N$ is set to equal a value of $1$. \\}
...
\note{However, as we will see in the numerical experiments, their additional cost of pooling inner replications may offset their benefit of fast convergence rate. \\}
...
\note{Let's first define the convergence in MSE and the convergence in probabilistic order. \\}
\note{The convergence in MSE is the convergence of the MSE of the risk measure estimator to the true risk measure, that is, $\hat{\rho}_{\Gamma}$ minus $\rho$ squared. \\}
\note{The convergence in probabilistic order is the convergence of the absolute error of the risk measure estimator to the true risk measure, that is, $|\hat{\rho}_{\Gamma} - \rho|$ to 0 in probability. \\}
\note{We show that the convergence rate of the MSE of the risk measure estimator implies the convergence rate of the absolute error in probabilistic order. \\}
\note{We will also show that the converse is not necessarily true.}
...
\note{The following numerical experiments test the empirical convergence rates of different nested simulation procedures across different risk measures, option types, asset dimensions, and asset models.}
...
\note{The following slides will show the sensitivity of the procedures to different factors. \\}
...
% \note{This figure shows the sensitivity of the procedures to the asset dimension. \\}
% \note{The asset dimension is the dimension of the risk factors $X$. \\}
% \note{Everything is the same as the previous figure, except that the asset dimension is varied from 1 to 2. \\}
% \note{The standard, regression, KRR, and likelihood ratio procedures are dimension-independent. \\}
% \note{The kernel smoothing procedure is sensitive to the asset dimension. \\}
% \note{Despite the fact that the kernel smoothing procedure has the same asymptotic convergence rate as before, it converges slower in practice. \\}
...
\begin{frame}{Total Computation Time}
    ...
    \note{This figure shows the total computation time for different procedures. \\}
    \note{It includes the time for both the simulation, training, validation, and everything else. \\}
    ...
    \note{The regression procedure and kernel smoothing procedure are the most efficient among metamodel-based procedures. \\}
    \note{Our implementation of the kernel smoothing procedure is efficient because we only run the cross-validation once for all macro replications. \\}
    \note{The likelihood ratio procedure and KRR procedure are more expensive due to their additional costs}
    \note{The likelihood ratio weight calculations increase quadratically with $\Gamma$. \\}
    \note{For KRR, the cost of cross-validation is so high that running it once still takes a very long time. \\}
...
\begin{frame}{Metamodel Performance}
    ...
    \note{We first consider the traditional regression metamodels. \\}
    \note{On the left is the QQ plot between the true loss labels and the predicted loss labels for MLR, and on the right is the QQ plot for QPR. \\}
    \note{MLR and QPR metamodels make inaccurate loss predictions. \\}
    \note{Feature engineering is hardly feasible for our 240-dimensional $X$. \\}
    \note{We will see whether deep neural networks can make more accurate loss predictions. \\}
...
\begin{frame}{Metamodel Performance on Different Datasets}
    ...
    \note{This suggests that the RNN and LSTM metamodels are able to cut through the simulation noise and learn the true feature-label relationship. \\}
    ...
    \note{We also find that the LSTM metamodel is able to cut through the noise in training labels and fit to the true relationship. \\}
    \note{We have another finding for RNN. RNN metamodels suffer from the vanishing gradient problem. \\}
    \note{This is why we make separate rows for well-trained RNNs and poorly-trained RNNs. \\}
    \note{We will discuss this in the next slide. \\}
...
\begin{frame}{Metamodel Performance}
    ...
    \note{We now consider the tail scenario identification for the two-stage procedure. \\}
    \note{On the left is the tail scenario identification for the traditional regression metamodel, and on the right is the tail scenario identification for the LSTM metamodel. \\}
    ...
    \note{This further motivates the use of LSTM metamodels over RNN metamodels. \\}    
...
\begin{frame}{Sensitivity Testing for DNNs}
    ...
    \note{We now consider the sensitivity testing for the DNN metamodels. \\}
    \note{We are interested in the sensitivity of the DNN metamodels to the noise level in training labels and the number of training samples. \\}
    \note{In a simulation experiment, we can vary them to see how the DNN metamodels perform. \\}
    \note{We consider the noise level in training labels by varying the number of inner replications $N'$. \\}
    \note{We consider the number of training samples by varying the number of outer scenarios $M$. \\}
...
\begin{frame}{Noise Tolerance of DNNs}
    ...
    \note{The closer the points are to the line, the better the model is. \\}
    \note{The high-capacity LSTM is better at low and medium noise levels, while the regular LSTM is better at high noise levels. \\}
...
\begin{frame}{Sensitivity of High-capacity LSTM}
    ...
    \note{The findings are similar for the high-capacity LSTM metamodel. \\}
    \note{We can find similar patterns as in the previous table. \\}
    \note{The model is more sensitive to $N'$ than $M$, unless $N'$ is very small. \\}
    \note{For a practical application, we recommend using the high-capacity LSTM metamodel with $N' = 10$ for almost all simulation budgets. }
...
\begin{frame}{Conclusion}
    ...
    \note{For the research directions. In the future, we can apply deep neural network metamodels to other actuarial applications.\\}
    \note{In the future, we can also apply transfer learning to speed up the training process for deep neural network metamodels.\\}
...
%     \note{This approach is effective when the source and target tasks are similar.\\}
...
%     \note{Multi-task learning trains a single model to perform multiple related tasks simultaneously.\\}
%     \note{This approach can improve generalization by leveraging the domain-specific information across tasks.\\}
%     \note{For VA contracts, we can train a model to handle multiple contract types or features.\\}
%     \note{The learning rate is a critical hyperparameter in transfer learning.\\}
%     \note{Typically, we use a smaller learning rate for fine-tuning compared to training from scratch.\\}
%     \note{The similarity between source and target tasks greatly impacts transfer learning effectiveness.\\}
%     \note{More similar tasks generally allow for more effective knowledge transfer.\\}
...
%     \note{The regime-switching GBM (RS-GBM) model captures more realistic market dynamics. \\}
...
%     \note{Now we examine transfer learning between different contract types, not just different features. \\}
%     \note{This is a more challenging transfer learning scenario as the contract structures differ significantly. \\}
%     \note{GMMB (Guaranteed Minimum Maturity Benefit) and GMWB (Guaranteed Minimum Withdrawal Benefit) have different risk profiles and valuation approaches. \\}
%     \end{frame}
