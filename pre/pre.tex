...
\note{In our problems, the 1-dimensional loss $L$ depends on the $d$-dimensional risk factors $X$. \\}
...
\note{The standard nested simulation procedure uses the inner sample mean under that scenario to estimate $L(X_i)$. \\}
...
\note{Pooling can happen in different ways, and the key idea is to use the information from different outer scenarios to improve the efficiency of nested simulation. \\}
\note{A supervised learning based procedure pools with these steps. \\}
\note{First, the user runs the standard procedure to generate a set of outer scenarios and inner sample means. \\}
\note{Then, they are treated as features and labels that train a metamodel. \\}
\note{Instead of using the inner sample mean, risk measures are estimated with the metamodel predictions. \\}
...
\note{These are the convergence rates of the MSE when the total simulation budget $\Gamma$ goes to infinity. \\}
...
\note{However, in our numerical experiments, we find that their additional cost of pooling is too high, which may offset their benefit of high asymptotic rates. \\}
...
\note{However, the analysis of KRR is different. \\}
...
\note{And we show that the convergence rate of $\Gamma^{-\xi}$ in MSE implies the convergence rate of $\Gamma^{-\frac{\xi}{2}}$ in probabilistic order. \\}
...
\note{Previously, we worked on the asymptotic convergence rates of different nested simulation procedures. \\}
\note{In practice, the total simulation budget is finite.}
\note{When the budget is finite, these theoretical rates may not be the only thing we should consider. \\}
...
\note{In the legend records the slopes of the lines and the corresponding asymptotic rates. \\}
\note{In this case, most procedures have their empirical convergence rates matching their asymptotic rates. \\}
...
\note{This figure shows the same experiment as the previous one, but we increase the asset dimension to 20. \\}
...
\note{The regression-based procedure is the only procedure that converges faster when the asset dimension is high. The reason is the regression-based procedure is particularly bad when we don't have enough data points at low $\Gamma$. We did a follow-up experiment to show regression converges in its asymptotic rate at high $\Gamma$. \\}
\note{Counter to what's on the graph, the likelihood ratio and KRR procedures are also sensitive to dimension. \\}
\note{While it is not shown in their empirical rate, due to their additional cost of pooling, we are not able to run these procedures when $\Gamma$ is large. \\}
...
\begin{frame}{Future Work}
    ...
    \note{Before here, we have shown that the regression-based procedure is the most cost-effective among all the procedures. \\}
    ...
    \note{However, the it is not suitable for modeling variable annuities. \\}
    \note{It is hard to find a good regression basis for high-dimensional risk factors. It will involve a lot of feature engineering and expert knowledge. \\}
    \note{In the next project, we will design a metamodel-based simulation procedures for variable annuities. \\}
...
\begin{frame}{Nested Simulation for Risk Management of VAs}
    ...
    \note{It is more expensive to run the standard nested for VAs. \\}
...
\note{After some numerical experiments, we found that the deep neural network metamodels are suitable for this problem. \\}
\note{We use LSTM for the sequential data. It can predict tail scenarios accurately, so we can use it to concentrate simulation budget on the tail scenarios. \\}
\note{However, it is not transparent. Many researchers are working on this topic. \\}
\note{This is where we can contribute to the field. \\}
\note{By using nested simulation as a data generating process, we can study the noise tolerance of DNNs. \\}
\note{We can control data quality and quantity by adjusting the simulation parameters. \\}
\note{By examining the performance of DNNs, we can provide direct evidence on the transparency of DNNs. }
...
\note{Our two-stage procedure is based on the idea of budget concentration. \\}
\note{In stage 1, we use a fraction of the simulation budget to run the standard nested simulation procedure. \\}
\note{We then train a metamodel using the scenarios and the inner sample means. \\}
\note{We use metamodel predictions to identify the tail scenario set that contains the $m$ largest predicted losses. \\}
\note{In stage 2, we run the standard nested simulation procedure on these predicted tail scenarios. \\}
...
\end{itemize}
    ...
    \note{In our experiments, we found some DNN metamodels that can make accurate loss predictions, so we propose a single-stage procedure. \\}
    \note{It shares the same idea of running the standard nested to get training data. \\}
    \note{After a metamodel is trained, the risk measure is estimated using the metamodel predictions directly. \\}
    \note{It is more efficient than the two-stage procedure because it avoids the stage 2 of the two-stage procedure. \\}
...
\begin{frame}{Experiment Setting}
    ...
    \note{The deep neural network metamodels are higher in capacity, but they are said to be more prone to overfitting. \\}
...
\begin{frame}{Metamodel Performance}
    ...
    \note{DNN metamodels are more flexible, and they can make more accurate predictions. \\}
...
\note{We have noisy training data and noisy test data. \\}
\note{We want to know what the metamodel learns from the noisy data. \\}
\note{We also want to know how well the metamodel can make accurate predictions. \\}
\note{In a simulation experiment, we have control over the noise level in the training data and the test data. \\}
\note{We also have access to the true relationship when we increase the number of replications. \\}
\note{We can use this true relationship to evaluate the performance of deep neural networks, which is usually not possible in practice. \\ }
\note{Instead of measuring the distance between the blue line and the orange dots, we can measure the distance between the blue line and the red line. }
...
\begin{frame}{Metamodel Performance on Different Datasets}
    ...
    \note{The average MSEs of metamodels on the training, test, and true data are reported in different colors. \\}
    \note{Note that the training and test labels are noisy due to simulation noise. This noise is less present in the true labels. \\}
    \note{First, we note that DNN metamodels has lower training errors than the traditional regression metamodels. \\}
    \note{This is expected because DNNs are more flexible. \\}
...
\begin{frame}{Safety Margin}
    ...
    \note{Coming back to the two-stage procedure, we need to choose a safety margin $m$ to identify the tail scenarios. \\}
    \note{Consider estimating the $95\%$ CVaR with 100,000 outer scenarios. \\}
    \note{On the top is the true tail and non-tail scenarios, and on the bottom is the predicted tail and non-tail. \\}
    \note{The 5\% predicted tails are not guaranteed to contain all the true tails. \\}
    \note{Therefore, we need to add a safety margin $m$. \\}
    \note{On the bottom, we add a 2\% safety margin. \\}
    \note{This adds 2,000 more scenarios to the inner simulations. \\}
    \note{We may add another 2\% safety margin to be more confident. \\}
    \note{This introduce a safety margin of $4\%$. In total, $9000$ predicted tails are used for extensive inner simulations in stage 2. \\}
    \note{This is a trade-off between accuracy and efficiency. \\}
    \note{For a regression metamodel, the safety margin needs to be large to identify all the tail scenarios. \\}
...
\begin{frame}{Metamodel Performance}
    ...
    \note{We now consider the precentage of tail match and the CVaR estimates for the two-stage procedure. \\}
    \note{On the left is the tail match plot, and on the right is the plot for CVaR. \\}
    \note{We compare the two-stage procedure with the standard procedure. \\}
    \note{When colored lines cross the grey dashed line, it means that the metamodel is able to identify the true tails as well as the standard procedure. \\}
    \note{And it should be able to estimate the CVaR as accurately. \\}
...
\begin{frame}{Sensitivity Testing for DNNs}
    ...
    \note{We study this phenomenon by testing the sensitivity of the DNN metamodels to the noise level in training labels and the number of training samples. \\}
    \note{In a simulation experiment, we have the flexibility to vary them. \\}
    \note{Data quality is controlled by $N'$, and data quantity is controlled by $M$. \\}
    \note{We consider two LSTM metamodels of different capacities. \\}
    \note{The high-capacity LSTM has 10 times more parameters than the regular LSTM.}
...
\begin{frame}{Single-Stage Procedure}
    ...
    \note{We compare it with the two-stage procedure using regular LSTM metamodel with no safety margin. So we are using the 5\% metamodel predicted tails to run extensive simulation in stage 2. \\}
...
\begin{frame}{Future Directions}
    ...
    \note{In this project, we demonstrate that LSTMs can effectively learn complex hedging strategies even with noisy training data.\\}
    \note{We can apply deep neural network metamodels to other actuarial products.\\}
    \note{However, DNNs are data-hungry. We may not have enough time to simulate enough data for new products.\\}
    \note{That's where transfer learning can help us to speed up the training process for contracts with not enough data.}
...
\begin{frame}{Transfer Learning for Rapid Adaptation of DNN Metamodels}
    ...
    \note{The difference is that we want to use transfer learning to speed up the training process when the amount of data is limited.\\}
    \note{This can be beneficial for the practical applications.\\}
    \note{Firstly, new VA contracts are continuously issued.\\}
    \note{Secondly, the computational budget is limited for simulation.\\}
    \note{With transfer learning, we can pre-train the deep neural network metamodel on a large dataset of VA contracts with abundant simulation data.\\}
    \note{Then, we can fine-tune the metamodel on new VA contracts with limited data.\\}
    \note{The shared features between VA contracts can be leveraged to speed up the training process.\\}
    \note{We can also try transferring knowledge to other contract types.}
...
\begin{frame}{Transfer Learning Framework}
    ...
    \note{For a VA dynamic hedging problem, our labels are the contract losses for different scenarios.\\}
    \note{The source domain typically has abundant data - this could be existing VA contracts with extensive simulation done already.\\}
    \note{The target domain, new VA contracts may have limited data.\\}
    \note{Our goal is to leverage knowledge from the source domain to improve performance in the target domain.\\}
    \note{This is particularly valuable when simulation data is expensive to generate for new contracts.\\}
    \note{It allows us to maintain accuracy while significantly reducing computational requirements.}
...
\begin{frame}{Transfer Learning Techniques}
    ...
    \begin{itemize}
        ...
        \item   Target domain (2000 samples): GMMB and GMWB with dynamic lapse 
    \end{itemize}

    \note{There are several ways to transfer knowledge.}
    ...
    \note{This is another attempt to use simulation as data generating processes to examine deep neural networks. \\}
    \note{By examining the transfer learning performance, we can gain insights into which parts of the neural network are transferable.\\}
    \note{Or more generally, which layers of the neural network learn which features of the VA contracts. }
...
\begin{frame}{Results: Accuracy Comparison}
    ...
    \note{Extensive training with full data achieves the best performance. \\}
    \note{Without transfer learning, training from scratch on dynamic lapse data yields mediocre results. \\}
    \note{Transfer learning from static lapse models performs well. \\}
    \note{Transfer from no lapse models performs poorly. It demonstrates that negative transfer can occur. \\}
...
\begin{frame}{Learning Curves: Effect of Similarity}
    \begin{figure}[ht]
        ...
        \begin{subfigure}{0.48\textwidth}
            ...
            \caption{From no lapse}
        \end{subfigure}
        \begin{subfigure}{0.48\textwidth}
            \includegraphics[width=\textwidth]{./figure2b.png}
            \caption{From static lapse}
        \end{subfigure}
        \caption{Fine-tuning performance on GMMB with dynamic lapse}
    \end{figure}

    \begin{itemize}
        \item Similarity between source and target domain matters
        \item Negative transfer can happen for dissimilar tasks
    \end{itemize}
    
    \note{These figures further illustrate the same idea. \\}
...
\begin{frame}{Learning Curves: Effect of Freezing Layers}
    ...
    \begin{itemize}
        \item   When learning dynamic lapse, freezing the LSTM layers is suboptimal.
        \item   Freezing the fully connected layers leads to better performance.
    \end{itemize}

    \note{These figures show the effect of freezing layers on the convergence of the model. \\}
    \note{When learning dynamic lapse, freezing the LSTM layers leads to poor performance. \\}
    \note{Freezing the fully connected layers is better since we are learning a dynamic lapse feature that introduces another source of stochasticity. \\}
    \note{This counters the previous assumption that the early layers learn general features and should be freezed. \\}
    \note{We suspect that the LSTM layers capture the time series component of the data, and the fully connected layers learn to model contract losses. \\}
    \note{When new source of stochasticity is introduced, the LSTM layers are not able to capture it. \\}
    \note{But static lapse and dynamic lapse, they are essentially the same contract with different lapse rates. \\}
    \note{Also, I want to mention when transferring from no lapse to static lapse, freezing the LSTM layers is better.}
...
\begin{frame}{Metamodel Performance}
    ...
    \note{These figures examine the performance of transfer learning from GMMB to GMWB contracts. \\}
    \note{The source task is a GMMB, and the target task is a GMWB. Both with dynamic lapse. \\}
    ...
    \note{When combined with a 2-stage procedure, we are able to reach reasonable performance for GMWB contracts with limited data. }
...
\begin{frame}{Conclusions}
    ...
    We study supervised learning models with simulation data.

    \begin{itemize}
        \item   Additional cost of pooling may overweight benefits.
        \item   DNNs cut through the simulation noise.
        \item   Data quantity is more important once quality is good enough.
        \item   Transferable features in different layers.
    \end{itemize}
    ...
    \textbf{Future Work:}
    \begin{itemize}
        \item Examine other types of DNNs.
        \item Try multi-task learning.
    \end{itemize}
    
    \note{Our study of supervised learning models with simulation data revealed several key insights. \\}
    \note{We found that while pooling data can be beneficial, the additional computational costs may sometimes outweigh the advantages. \\}
    \note{Deep neural networks demonstrated remarkable ability to cut through the simulation noise. \\}
    \note{Our results suggest that once data quality reaches a certain threshold, increasing quantity becomes more beneficial. \\}
    \note{We also identified that different neural network layers contain different features that can be transferred between contract types. \\}
    \note{For future work, we plan to explore other types of DNN architectures in other actuarial applications. \\}
    \note{We also aim to implement multi-task learning approaches to simultaneously model multiple similar actuarial products. \\}
